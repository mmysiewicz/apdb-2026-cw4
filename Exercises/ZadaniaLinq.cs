...
public sealed class ZadaniaLinq
{
    ...
    public IEnumerable<string> Zadanie04_PierwszyPrzedmiotAnalityczny()
    {
        ...
        if (method == null)
        {
            string answear = "Taki przedmiot nie istnieje";
            return [answear];
        }
        else
        {
    ...
    public IEnumerable<string> Zadanie06_CzyWszyscyProwadzacyMajaKatedre()
    {
        var method = DaneUczelni.Prowadzacy
            .Count(e => !string.IsNullOrEmpty(e.Katedra));
        if (method == 0)
        {
            return ["1"];
        }
    ...
    public IEnumerable<string> Zadanie12_ParyStudentPrzedmiot()
    {
        var method = DaneUczelni.Zapisy
            .SelectMany(
                ...
                (z, s) => new {z, s})
            .SelectMany(
                zs => DaneUczelni.Przedmioty
                    .Where(p => p.Id == zs.z.PrzedmiotId),
    ...
    public IEnumerable<string> Zadanie14_SredniaOcenaNaPrzedmiot()
    {
        var method = DaneUczelni.Zapisy
            ...
            .GroupBy(zp => zp.p.Nazwa)
            .Select(e => $"{e.Key}, {e.Average(zp => zp.z.OcenaKoncowa)}");
    ...
    public IEnumerable<string> Zadanie15_ProwadzacyILiczbaPrzedmiotow()
    {
        var method = DaneUczelni.Prowadzacy
            ...
            .Select(e => $"{e.Imie}, {e.Nazwisko}, {e.Liczba}");
    ...
    public IEnumerable<string> Zadanie16_NajwyzszaOcenaKazdegoStudenta()
    {
        var method = DaneUczelni.Studenci
            .SelectMany(s => DaneUczelni.Zapisy
                ...
                (s, z) => new {s, z})
            .GroupBy(
                sz => new {sz.s.Imie, sz.s.Nazwisko},
                sz => sz.z.OcenaKoncowa)
            .Select(e => $"{e.Key.Imie}, {e.Key.Nazwisko}, {e.Max()}");
    ...
    public IEnumerable<string> Wyzwanie02_PrzedmiotyStartujaceWKwietniuBezOcenKoncowych()
    {
        var method = DaneUczelni.Przedmioty
            .Join(DaneUczelni.Zapisy,
                ...
                z => z.PrzedmiotId,
                (p, z) => new { p, z })
            .Where(pz => pz.p.DataStartu.Month == 4 
                         && pz.p.DataStartu.Year == 2026)
            .GroupBy(pz => pz.p.Nazwa)
            .Where(e => e.Count(pz => pz.z.OcenaKoncowa != null) == 0)
            .Select(e => $"{e.Key}");
